...
class Plotter:
	...
	def batches_tk(self, *args):
		...
		self.tab_batches_commit_batch_but = Button(self.tab_batches_split_list_frame, text='Commit Batch to Database', command=self.tab_batches_commit)
		self.tab_batches_commit_batch_but.grid(row=3, column=0, sticky=W, columnspan=25)
		
		self.tab_batches_status_lab_var = StringVar()
		self.tab_batches_status_lab = Label(self.tab_batches_split_list_frame, textvariable=self.tab_batches_status_lab_var)
		self.tab_batches_status_lab.grid(row=4, column=0, sticky=W, columnspan=10)
		self.tab_batches_status_lab_var.set('Status:')
	
		self.tab_batches_import_but = Button(self.tab_batches_split_list_frame, text='Import batch JV data', command=self.tab_batches_jv_import)
		self.tab_batches_import_but.grid(row=5, column=0, sticky=W, columnspan=25)
		
		self.tab_batches_print_but = Button(self.tab_batches_split_list_frame, text='Print Device Labels', command=self.tab_batches_print_label)
		...
		self.tab_batches_notes_frame.grid(row=2, column=0, sticky=W)
	...
	def stacks_tk(self, *args):
		...
		self.tab_stack_listbox = Listbox(self.tab_stack_listbox_frame, yscrollcommand=self.tab_stack_listbox_vsb.set, exportselection=False, width=60)
	...
	def tab_stack_update_top_elec_drop_down(self,*args):
		result = self.select_many_things_where(self, 'elec_name', 'electrode_patterns', 'elec_name', 'top')
		if result:
			self.tab_stack_top_elec_list = []
			for i in result:
	...
	def tab_stack_update_bot_elec_drop_down(self,*args):
		result = self.select_many_things_where(self, 'elec_name', 'electrode_patterns', 'elec_name', 'bottom')
		if result:
			self.tab_stack_bot_elec_list = []
			for i in result:
	...
	def tab_form_print(self, *args):
		...
		qr = """^XA^FO50,50^BQ,2,2^FDQA,""" + 'ID: %s, Date: %s-%s-%s' % (form_id_select_result,self.tab_form_spin_year.get(), self.tab_form_spin_month.get(), self.tab_form_spin_day.get())+ """^FS"""

		line0 = """^FO40,20^FD"""+"%s" % (str(self.var_form_name.get())[0:25])+"""^FS"""
		line1 = """^FO40,30^FD"""+"%s" % (str(self.var_form_name.get())[25:50])+"""^FS"""
		line2 = """^FO40,40^FD"""+"%s" % (str(self.var_form_name.get())[50:75])+"""^FS"""
		line3 = """^FO120,55^FD"""+"ID %s" % (form_id_select_result)+"""^FS"""
		line4 = """^FO120,65^FD"""+"Date %s%s%s" % (self.tab_form_spin_year.get(), self.tab_form_spin_month.get(), self.tab_form_spin_day.get())+"""^FS"""
		line5 = """^FO120,75^FD"""+"Waste %s" % (str(self.tab_form_waste_box.get())[0:8])+"""^FS"""
		lab_end = """^XZ"""
